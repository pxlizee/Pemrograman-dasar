...
AVAILABLE_ROOMS = {'A': {'floors': list(range(1, 6)), 'rooms': ['-']}, 'B': {'floors': list(range(1, 7)), 'rooms': ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H']}}
...
KOLOM_WAJIB = ['HARI', 'DOSEN', 'MATAKULIAH', 'SEMESTER', 'SKS', 'KELAS', 'MODE', 'PRODI', 'GEDUNG', 'LANTAI', 'RUANGAN', 'MULAI', 'SELESAI', 'TANGGAL_DIBUAT']
...
class ReservationApp:
    def __init__(self, root):
        ...
        widget_configs = {
            ...
            'LANTAI': {'widget': ttk.Combobox, 'options': {}}, 'RUANGAN': {'widget': ttk.Combobox, 'options': {}},
    ...
    def save_data_auto(self):
        """Fungsi ini untuk penyimpanan otomatis setelah ada perubahan."""
        try:
    ...
    def export_to_excel(self):
        """Membuka dialog 'Save As' untuk mengekspor data ke file .xlsx."""
        if self.df.empty:
    ...
    def on_mode_select(self, event=None):
        ...
        if mode == 'ONLINE':
            for widget in location_widgets:
                widget.set('-'); widget.config(state='disabled')
        else:
            self.entries['GEDUNG'].config(state='readonly'); self.entries['LANTAI'].config(state='readonly'); self.entries['RUANGAN'].config(state='readonly')
            if self.entries['GEDUNG'].get() == '-':
                self.entries['GEDUNG'].set(''); self.update_location_options()

    def update_location_options(self, event=None):
        selected_gedung = self.entries['GEDUNG'].get()
        self.entries['LANTAI'].set(''); self.entries['RUANGAN'].set('')
        if selected_gedung and selected_gedung != '-':
            self.entries['LANTAI']['values'] = AVAILABLE_ROOMS[selected_gedung]['floors']
            room_options = AVAILABLE_ROOMS[selected_gedung]['rooms']
            self.entries['RUANGAN']['values'] = room_options
            if len(room_options) == 1:
                self.entries['RUANGAN'].set(room_options[0]); self.entries['RUANGAN'].config(state='disabled')
        else:
            self.entries['LANTAI']['values'] = []; self.entries['RUANGAN']['values'] = []
    ...
    def on_item_select(self, event):
        ...
        for key in KOLOM_FORM:
            ...
            if isinstance(self.entries[key], ttk.Combobox): self.entries[key].set(value)
            else: self.entries[key].insert(0, value)
        self.on_mode_select(); self.update_location_options()
    ...
    def _get_and_validate_form_data(self):
        ...
        data['RUANGAN_UPPER'] = data['RUANGAN'].upper() if mode == 'OFFLINE' else '-'
    ...
    def update_reservation(self):
        ...
        if data.get('MODE') == 'OFFLINE':
            ...
            if not valid: messagebox.showerror("Jadwal Bentrok", msg); return
        for col_name in KOLOM_FORM:
            value = data.get(col_name)
            if col_name in ['LANTAI', 'SEMESTER', 'SKS']: self.df.loc[df_index_to_update, col_name] = int(value) if value != '-' else 0
            elif col_name in ['HARI', 'KELAS', 'GEDUNG', 'RUANGAN', 'MODE']: self.df.loc[df_index_to_update, col_name] = value.upper() if value != '-' else '-'
            else: self.df.loc[df_index_to_update, col_name] = value
